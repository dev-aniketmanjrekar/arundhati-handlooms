...
app.post('/api/orders', async (req, res) => {
    ...
    const { items, totalAmount, shippingAddress, shippingPincode } = req.body;
    ...
    try {
        ...
        try {
            const [orderResult] = await connection.query(
                'INSERT INTO orders (user_id, total_amount, shipping_address, shipping_pincode) VALUES (?, ?, ?, ?)',
                [userId, totalAmount, shippingAddress, shippingPincode]
            );
            ...
            // Auto-update best sellers after order
            try {
                ...
                // Don't fail the order if best seller update fails
            }
...
app.post('/api/inquiries', async (req, res) => {
    const { name, email, phone, inquiryType, productInterest, customProductDetails, message } = req.body;

    try {
        const [result] = await pool.query(
            'INSERT INTO inquiries (name, email, phone, inquiry_type, product_interest, custom_product_details, message) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [name, email, phone, inquiryType, productInterest, customProductDetails, message]
        );
...
// Get all products (Admin view might need more details or raw list)
...
app.post('/api/admin/products', isAdmin, async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.query(
            'INSERT INTO products (name, category, price, description, image_url, images, color, fabric_type, sku, size, wholesale_price, retail_price, discount_percent, is_new, is_best_seller, stock_quantity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [name, category, finalPrice, description, image_url, JSON.stringify(images), color, fabric_type, sku, size, wholesale_price, finalRetailPrice, discount_percent || 20, is_new, is_best_seller, stock_quantity]
        );
        res.status(201).json({ message: 'Product created', id: result.insertId, sku });
    } catch (error) {
        console.error(error);
        if (error.code === 'ER_DUP_ENTRY') {
            res.status(400).json({ message: 'SKU already exists. Product names with same color must be unique.' });
        } else {
            res.status(500).json({ message: 'Server error' });
        }
    }
